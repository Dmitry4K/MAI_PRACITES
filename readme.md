...
Здесь будет выкладываться различная информация по практике
# MiniOS
## Файлы репозитория
...
_fs.h/.c_ - виртуальная файловая система

_initrd.h/.c_ - функуции работы с модулем initrd

_result.zip_ - архив с итоговой сборкой, исходниками и новыми файлами
...
В итоге всех махинаций получается образ MiniOS - hdd.img.
...
Для его подключения нужно добавить initrd.img к модулю hdd.img, добавив несколько строк в makefile, затем в файлу menu.lst указать этот модуль.
...
```
read_type_t, write_type_t, open_type_t и т.д. ничто иное как указатели на функции открытия, чтения и закртия файлов. При иницилизации файловой системы в эти поля должны быть помещены указатели на соотвествующие функции, которые будут вызываться файловой системой. Это происходит здесь (отмечено <----):
...
Также стоит отметить, что в глобальную область видимости помещаются следующие данные:
